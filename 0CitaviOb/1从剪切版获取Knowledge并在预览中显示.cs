...
public static class CitaviMacro
{
    ...
    public static async Task Main() 
    {
		...
		DebugMacro.WriteLine(string.Format("找到知识条目数: {0}", foundKnowledgeItems.Count.ToString()));
        ...
        // 4.4. 设置活动参考文献和知识条目
        // 为了让PDF预览能正确加载，需要设置当前活动的参考文献和知识条目。
        // 我们选择列表中的第一个知识条目作为跳转目标。
        mainForm.ActiveWorkspace = MainFormWorkspace.ReferenceEditor;
        // 4.3. 确保中心面板可见，并切换到“位置”标签页
        // 如果中心面板被折叠，则展开它，以便用户能看到PDF预览。
        if (mainForm.IsCenterPaneCollapsed)
        {
            mainForm.IsCenterPaneCollapsed = false;
        }
        // 将活动标签页设置为“位置”（即Quotations）。
